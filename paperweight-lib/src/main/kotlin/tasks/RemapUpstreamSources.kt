...
abstract class RemapUpstreamAction : WorkAction<RemapUpstreamParams> {
    override fun execute() {
        val mappingSet = MappingFormats.TINY.read(
            ...
            parameters.fromNamespace.get(),
            parameters.toNamespace.get()
        )
        ...
        Mercury().let { mercury ->
            ...
            try {
                ...
                parameters.remappedDir.path.deleteRecursively()
                parameters.remappedDir.path.createDirectories()
                tempOut.copyRecursivelyTo(parameters.remappedDir.path)
            } finally {
...
interface RemapUpstreamParams : WorkParameters {
    ...
    val remappedDir: RegularFileProperty
}
