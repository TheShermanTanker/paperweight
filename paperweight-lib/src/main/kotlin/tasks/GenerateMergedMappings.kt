...
@UntrackedTask(because = "Mappings from Spigot can change at any time")
abstract class GenerateMergedMappings : JavaLauncherTask() {
    ...
    @get:InputDirectory
    @get:PathSensitive(PathSensitivity.NONE)
    abstract val spigotMappingsDir: DirectoryProperty
    ...
    @TaskAction
    fun run() {
        val initialMergedCache = createTempFile("fixed-spigot-mappings", "tiny")
        val spigotMemberCache = createTempFile("patched-spigot-member-mappings", "csrg")
        try {
            ...
            val buildDataInfoRaw: RegularFileProperty = project.objects.fileFrom(spigotMappingsDir, "info.json")
            val buildDataInfo: Provider<BuildDataInfo> = project.contents(buildDataInfoRaw) {
                gson.fromJson(it)
            }

            val mappingsDir = objects.dirFrom(spigotMappingsDir, "mappings")

            appendLines(mappingsDir.file(buildDataInfo.map { it.memberMappings }).path, spigotMemberMappingsPatch.pathOrNull, spigotMemberCache)
            ...
            val memberMappingSet = MappingFormats.CSRG.createReader(spigotMemberCache).use { it.read() }
            ...
            MappingFormats.TINY.write(fixedSpigotMappings, initialMergedCache, OBF_NAMESPACE, MERGED_NAMESPACE)
            ...
            queue.submit(CleanMappingsAction::class) {
                ...
                inputMappings.set(initialMergedCache)
            ...
            if(mergedMappingsPatch.pathOrNull != null) {
                queue.submit(CleanMappingsPatchAction::class) {
                    sourceJar.set(this@GenerateMergedMappings.mojangMappedJar.path)
                    libraries.from(this@GenerateMergedMappings.libraries.files)
                    inputMappings.set(mergedMappingsPatch.path)
            ...
            queue.submit(CleanSourceMappingsAction::class) {
                sourceJar.set(this@GenerateMergedMappings.mojangMappedJar.path)
                libraries.from(this@GenerateMergedMappings.libraries.files)
                inputMappings.set(patchedMojangToMergedMappings.path)
                fromNamespace.set(DEOBF_NAMESPACE)
                toNamespace.set(MERGED_NAMESPACE)
            ...
            initialMergedCache.deleteForcefully()
            spigotMemberCache.deleteForcefully()
        }
    ...
    abstract class CleanMappingsPatchAction : WorkAction<CleanMappingsPatchAction.Parameters> {
        ...
        override fun execute() {
            val mappings = MappingFormats.CSRG.read(parameters.inputMappings.path)
    ...
    abstract class CleanMappingsAction : WorkAction<CleanMappingsAction.Parameters> {
        ...
        override fun execute() {
            val mappings = MappingFormats.TINY.read(parameters.inputMappings.path, parameters.fromNamespace.get(), parameters.toNamespace.get())
    ...
    abstract class GenerateMergedMappingsPatchAction : WorkAction<GenerateMergedMappingsPatchAction.Parameters> {
        ...
        override fun execute() {
            val mappings = MappingFormats.TINY.read(parameters.inputMappings.path, DEOBF_NAMESPACE, MERGED_NAMESPACE)
